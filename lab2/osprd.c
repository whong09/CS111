...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
	...
	// Your code here.
        ...
        {
 	  eprintk("Trying to write to nonexistent sectors\n");
          end_request(req,0);
	}
	...
	if (cmd == OSPRDIOCACQUIRE) {
          ...
          if(filp_writable)
	  {
		...
		//r = -ENOTTY;
          ...
          {
	    osp_spin_lock(&d->mutex);
	    if(d->num_write_locks != 0 || d->num_read_locks != 0 || d->ticket_head != d->ticket_tail)
	    {
	      r = -EBUSY;
            }
            else
          ...
          {
            osp_spin_lock(&d->mutex);
            if(d->num_write_locks != 0 || d->ticket_head != d->ticket_tail)
	    {
	      r = -EBUSY;
            }
            else
