...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if(entry_oi->oi_ftype == OSPFS_FTYPE_REG)
			ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, od->od_ino, OSPFS_FTYPE_REG);
		else if(entry_oi->oi_ftype == OSPFS_FTYPE_DIR)
			ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, od->od_ino, OSPFS_FTYPE_DIR);
		else if(entry_oi->oi_ftype == OSPFS_FTYPE_SYMLINK)
			ok_so_far = filldir(dirent, od->od_name, strlen(od->od_name), f_pos, od->od_ino, OSPFS_FTYPE_SYMLINK);
		f_pos++;
	}
...
allocate_block(void)
{
	...
	{
		uint32_t *vector = ospfs_block(free_block);
		uint32_t offset = 0;
		while(offset < OSPFS_BLKSIZE)
		{
	...
	return 0;
	//couldn't allocate a block - disk is full
}
...
{
	/* EXERCISE: Your code here */
	bitvector_clear(ospfs_block(OSPFS_FREEMAP_BLK+blockno/OSPFS_BLKSIZE), blockno%OSPFS_BLKSIZE);
	//vector = freemap + blockno / blocksize & offset = blockno % blocksize
}
...
add_block(ospfs_inode_t *oi)
{
	...
	if(indir2_index(n) == 0) //indirect^2 block
	{
		...
		if(oi->oi_indirect2 == NULL)
		{
			...
			i2_vector = ospfs_block(oi->oi_indirect2);
			memset(i2_vector, 0, OSPFS_BLKSIZE);
		}
		else
			i2_vector = oi->oi_indirect2;
		uint32_t i2_off = indir_index(n)-1;
		if(i2_vector[i2_off] == NULL)
		{
	...
	else if(indir_index(n) == 0) //indirect block
	{
		...
		if(oi->oi_indirect == NULL)
		{
			...
			i_vector = ospfs_block(oi->oi_indirect);
			memset(i_vector, 0, OSPFS_BLKSIZE);
		}
		else
			i_vector = oi->oi_indirect;
		uint32_t i_off = direct_index(n);
		if((i_vector[i_off] = allocate_block()) == 0 && allocated[0])
		{
			free_block(allocated[0]);
			oi->oi_indirect = NULL;
			return -ENOSPC;
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		if(count - amount < n)
			n = count - amount;
		copy_to_user(buffer, &data[*f_pos%OSPFS_BLKSIZE], n);
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	{
		int s = change_size(oi, *f_pos+count);
		if(s != 0)
			return s;
	}
	...
	while (amount < count && retval >= 0) {
		...
		/* EXERCISE: Your code here */
		/*retval = -EIO; // Replace these lines //replaced
		goto done;*/
		n = OSPFS_BLKSIZE - (*f_pos % OSPFS_BLKSIZE);
		if(count - amount < n)
			n = count - amount;
		copy_from_user(&data[*f_pos%OSPFS_BLKSIZE], buffer, n);
